...
const globalState = globalThis as typeof globalThis & { __paperPositions?: Map<string, PaperPosition>; __paperMonitor?: ReturnType<typeof setInterval>; __buyAlertMonitor?: ReturnType<typeof setInterval>; __buyAlerts?: Map<string, { signature: string; sentAt: number }> };
...
async function checkPosition(position: PaperPosition) {
  ...
  let reason: string | null = price <= position.stop ? `Stop Loss reached at ${price}` : price >= position.target ? `Take Profit reached at ${price}` : null;
  if (!reason) {
    const scan = await scanAllMarkets();
    const current = scan.candidates.find((candidate) => candidate.symbol === position.symbol);
    if (!current || current.preference !== "BUY") reason = "The qualified BUY setup is no longer active";
  }
  if (!reason) return;
...
async function monitorBuyOpportunity() {
  ...
  if (!candidate || positions.values().some((position) => position.symbol === candidate.symbol && position.status !== "CLOSED")) return;
